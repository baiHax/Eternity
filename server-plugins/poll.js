...
class Poll {
	...
	generateVotes(num) {
		let output = '<div class="infobox"><details><summary style="margin: 2px 0 5px 0"><span style="border:1px solid #6A6;color:#484;border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> Encuesta-' + this.pollArray[num].pollNum + '</span> <strong style="font-size:11pt">' + this.getQuestionMarkup(num) + '</strong></summary>';
		...
		output += '<div style="margin-top: 7px; padding-left: 12px"><button value="/poll results ' + this.pollArray[num].pollNum + '" name="send" title="Ver resultados - you will not be able to vote after viewing results"><small>(View results)</small></button></div>';
		output += '</details></div>';
	...
	generateResults(ended, option, num) {
		let icon = '<span style="border:1px solid #' + (ended ? '777;color:#555' : '6A6;color:#484') + ';border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> ' + (ended ? "Encuesta-" + this.pollArray[num].pollNum + " ended" : "Encuesta-" + this.pollArray[num].pollNum) + '</span>';
		...
		while (!i.done) {
			...
			output += '<div style="margin-top: 3px">' + i.value[0] + '. <strong>' + (i.value[0] === option ? '<em>' : '') + this.getOptionMarkup(i.value[1], num) + (i.value[0] === option ? '</gem>' : '') + '</strong> <small>(' + i.value[1].votes + ' voto' + (i.value[1].votes === 1 ? '' : 's') + ')</small><br /><span style="font-size:7pt;background:' + colors[c % 3] + ';padding-right:' + (percentage * 3) + 'px"></span><small>&nbsp;' + percentage + '%</small></div>';
			i = iter.next();
			c++;
		}
	...
	poll: {
		...
		new: function (target, room, user, connection, cmd, message) {
			...
			if (options.length > 20) {
				return this.errorReply("Too many options for poll (maximum is 20).");
			}
